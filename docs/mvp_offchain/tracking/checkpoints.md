...
- 2026-03-14: `W2-05` closed `DONE`. Implemented quiz pool generator backend (`collector/quiz_pool_generator.py`) with Ollama `qwen2.5:3b`, idempotent versioning (`quizzes.pool_signature`, `quizzes.version`), persistence in `quizzes` + `quiz_questions`, and story-level quiz state (`stories.quiz_status`, `quiz_unavailable_reason`, `quiz_pool_version`, `quiz_updated_at`). QA evidence on `data/polinews_w202_realqa.db`: publishable stories `49`, `quiz_available` `44`, coverage `89.8%` (`>=80%` target), unavailable reason distribution `quiz_generation_failed=5`. Details in `tracking/w2_05_quiz_pool_qa_report.md`.
